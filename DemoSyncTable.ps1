...
$Endpoint = New-UDEndpoint -Schedule $Schedule -Endpoint {
    ...
    $Cache:process = Get-Process | select -first 5 | Out-UDTableData -Property @("name", "id")
    $Cache:service = Get-Service | select -first 5 | Out-UDTableData -Property @("DisplayName", "status")
}
